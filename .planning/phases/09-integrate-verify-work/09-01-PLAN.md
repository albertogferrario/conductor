...
<objective>
Create Conductor-compliant foundation for verify-work: workflow, template, and refactored command.

Purpose: Align community-contributed verify-work command with Conductor patterns (workflow delegation, templates, terse steps).
...
**Pattern references (existing Conductor commands to match):**
@commands/gsd/discuss-phase.md
@commands/gsd/consider-issues.md

**Pattern references (existing Conductor workflows to match):**
@conductor/workflows/discuss-phase.md

**Pattern references (existing Conductor templates to match):**
@conductor/templates/context.md
@conductor/templates/issues.md
</context>
...
<task type="auto">
  ...
  <files>conductor/workflows/verify-work.md</files>
  <action>
Create workflow following Conductor patterns from discuss-phase.md:
...
- Reference template for issues format: `@~/.claude/conductor/templates/uat-issues.md`
...
- Offer next actions based on results
  ...
  <verify>File exists at conductor/workflows/verify-work.md with proper XML structure</verify>
  <done>Workflow file created with Conductor-compliant structure, all logic preserved</done>
</task>

<task type="auto">
  <name>Task 2: Create UAT issues template</name>
  <files>conductor/templates/uat-issues.md</files>
  <action>
...
- Used as input for /dev:plan-fix command
- No "Closed Enhancements" section (issues are fixed via FIX.md plan, then file is historical record)
  </action>
  <verify>File exists at conductor/templates/uat-issues.md with proper template structure</verify>
...
<task type="auto">
  ...
  <action>
Refactor to match Conductor command patterns from discuss-phase.md:
- Add `name: dev:verify-work` to frontmatter (already present)
- Update execution_context to reference workflow: `@~/.claude/conductor/workflows/verify-work.md`
- Add template reference: `@~/.claude/conductor/templates/uat-issues.md`
...
```
  ...
  <verify>Command file matches Conductor patterns, references workflow and template</verify>
  <done>Command refactored to delegate to workflow, follows Conductor conventions</done>
</task>
...
- [ ] `conductor/workflows/verify-work.md` exists with proper XML structure
- [ ] `conductor/templates/uat-issues.md` exists with template format
...
- Workflow follows Conductor patterns (terse steps, XML structure)
- Template follows Conductor patterns (examples, guidelines)
